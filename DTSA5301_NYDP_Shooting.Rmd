...
Reproducibility on data analysis with R is feasible, but the fundamental knowledge, skills, and careful implementations are required.
...
We can see no significant difference between before and after the Pandemic in any graphs for  females and males.
...
Conversely, I can also say that the NYPD Shooting Incident Data used in this analysis does not indicate that any particular factor rooted in a specific race or sex is responsible for the surge in New York shootings.

Rather, other factors (social unrest, unemployment, etc.) may be the cause; thus, a larger-scale analysis that combines the data used in this analysis and other data is required.
...
Speaking of the findings from the data analysis, I have found nothing significant that could be attributed to the spike in the NY shootings in May 2020 from any of the attributes of victims and perpetrators.

As mentioned, other factors (social unrest, unemployment, etc.) may be the reason. Therefore, a larger-scale analysis with the NYPD Shooting Incident Data and other reliable data is desired.

*****

## Bias Identification

Eliminating bias in data analysis is challenging; this report is no exception. I assumed that the COVID-19 and/or the BLM Movement Pandemic caused the May 2020 spike in New York shootings and attempted to analyze the specific factors further.
However, that assumption itself, while confident based on the majority of people's experience, is still a bias based on preconceived notions.
However, even if the initial hypothesis contains bias, if we proceed with a thorough analysis of the specific factors, we will eventually arrive at the actual root cause of the phenomenon.
So, I considered that assumption to be nothing more than a trigger and be acceptable in this report.

Other biases could also be contained in the shooting data itself. For example, "race" in the NYPD shooting count data was classified into six categories, except those marked unknown.
In reality, however, the classification of New Yorkers into six races needs to be revised and improved. Biases already infused in the first place during the compilation are even harder to correct or eliminate. Although this report does not explicitly address potential biases in the data, it is hoped that more reliable data will be collected and a more sophisticated analysis will be attempted.
